...
#--quantum: fragment Hamiltonians -> glue all fragment circuits -> send to hardware -> counts per fragment
...
def run_hybrid_cycle(mo):
    ...
    for ifrag, (h0, h1s, h2) in enumerate(fragment_hamiltonians(las, mo)):
        qcs.append(LUCJ_circuit(num_orbitals[ifrag],num_elec_a[ifrag],num_elec_b[ifrag],spin_sq,h1s[0,:,:],h2))
