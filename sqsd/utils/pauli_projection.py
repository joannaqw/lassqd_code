"""
Functions for projecting arbitrary strings of Pauli operators.

.. currentmodule:: sqsd.utils.pauli_projection

.. autosummary::
   :toctree: ../stubs/
   :nosignatures:

   matrix_elements_pauli_string
   unique_bitstrings_sorted
"""
...
def sort_and_remove_duplicates(bitstring_matrix: np.ndarray, inplace: bool = True) -> np.ndarray:
    ...
    Returns:
        Sorted version of X without repeated rows.
    """
    if not inplace:
...
def matrix_elements_from_pauli_string(
    ...
    """
    Finds the matrix elements of a Pauli operator in the subspace defined by the bitstrings.

    NOTE: The bitstrings in the ``bitstring_matrix`` must be sorted and unique according
