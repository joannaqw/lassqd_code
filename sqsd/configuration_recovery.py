...
.. currentmodule:: sqsd.configuration_recovery
...
def recover_configurations(
    ...
    Args:
        ...
        avg_occupancies: A 1D array containing the mean occupancy of each orbital. This is
            typically derived from the ground state estimated during QSCI. It is assumed
            that ``avg_occupancies[i]`` corresponds to the orbital represented by columns
            ``i`` and ``i + # orbitals`` in ``bitstring_matrix``.
...
def _bipartite_bitstring_correcting(
    ...
    Args:
        ...
        avg_occupancies: A 1D array containing the mean occupancy of each orbital. This is
            typically derived from the ground state estimated during QSCI.
        hamming_left: The target hamming weight used for the left half of the bitstring
        hamming_right: The target hamming weight used for the right half of the bitstring
        rand_seed: A seed to control random behavior
