...
from mrh.my_dmet import pyscf_rhf, pyscf_mp2, pyscf_cc, pyscf_casscf, pyscf_fci #, chemps2
...
from mrh.util.rdm import Schmidt_decomposition_idempotent_wrapper, get_1RDM_from_OEI, get_2CDM_from_2RDM
...
class fragment_object:
    ...
    def do_Schmidt_LASSCF (self, oneRDM_loc, all_frags, loc2wmcs):
        ...
        if self.imp_solver_name == 'dummy RHF':
            ...
            return
        '''
        if 'dummy' not in self.imp_solver_name:
        ...
        project_operator_into_subspace (self.oneRDM_loc, self.loc2core)
        ...
        0 if self.active_space is None else self.active_space[0]
        ...
        if norbs_hessbath and self.add_virtual_bath and self.imp_solver_name != 'dummy RHF' and self.norbs_as:
            ...
            np.append (loc2canon_imp[:,:norbs_inac_imp], loc2canon_imp[:,norbs_occ_imp:], axis=1)
    ...
    def gradient_for_virtbath (self, loc2virtbath, oneRDM_loc, loc2wmcs, fock=None):
        ...
        (self.nelec_imp - self.nelec_as) // 2
    ...
    def impurity_molden (self, tag=None, canonicalize=False, natorb=False, molorb=False, ene=None, occ=None):
        tag = '.' if tag is None else '_' + str (tag) + '.'
