import numpy as np
import itertools
from mrh.util.basis import represent_operator_in_basis, is_basis_orthonormal, measure_basis_olap, get_complementary_states, is_basis_orthonormal_and_complete
...
def examine_ifrag_olap (dmet_obj):
    ...
    for f1, f2 in itertools.combinations_with_replacement (frags, 2):
        if f1 is not f2:
            ...
            np.dot (f1.loc2emb[:,:f1.norbs_frag].T, f2.loc2emb[:,:f2.norbs_frag])
