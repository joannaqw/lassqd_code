...
from mrh.util.basis import get_subspace_symmetry_blocks
...
def solve (frag, guess_1RDM, chempot_imp):
    ...
    if not frag.mfmo_printed and frag.ints.mol.verbose:
    ...
    represent_operator_in_basis (twoCDM_amo, imp2amo.conjugate ().T)
...
def project_amo_manually (loc2imp, loc2gamo, fock_mf, norbs_cmo, dm=None):
    ...
    np.dot (amo2imp, amo2imp.conjugate ().T)
    '''
