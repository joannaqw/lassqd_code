...
from mrh.my_dmet import qcdmethelper
...
from mrh.util.la import matrix_eigen_control_options, assign_blocks_weakly
...
from mrh.util.basis import measure_basis_olap, is_basis_orthonormal
from mrh.util.basis import measure_subspace_blockbreaking
...
from mrh.my_dmet.debug import debug_Etot, examine_ifrag_olap, examine_wmcs
...
class dmet:
    ...
    def __init__( self, theInts, fragments, calcname='DMET', isTranslationInvariant=False, SCmethod='BFGS', incl_bath_errvec=True, use_constrained_opt=False, 
        ...
        if ( self.norbs_allcore > 0 ): # One or more impurities which do not cover the entire system
            assert( not self.TransInv ) # Make sure that you don't work translational invariant
    ...
    def makelist_H1( self ):
        ...
        if ( self.doDET ): # Do density embedding theory
    ...
    def doexact( self, chempot_frag=0.0 ):
        self.helper.construct1RDM_loc( self.doSCF, self.umat ) 
        self.energy = 0.0												
        self.spin = 0.0
...
#        return gradient
    ...
    def refrag_lowdin_active (self, loc2wmas, oneRDM_loc):
        ...
        for f in self.fragments:
            if f.loc2amo.shape[1] == 0:
                continue
            represent_operator_in_basis (np.dot (f.loc2amo, f.loc2amo.conjugate ().T), loc2wmas)
        ...
        represent_operator_in_basis (oneRDM_loc, np.concatenate (loc2amo, axis=1))
    ...
    def get_las_nos (self, **kwargs):
        ...
        kwargs['aobasis'] = True
        if 'loc2wmas' not in kwargs or kwargs['loc2wmas'] is None:
