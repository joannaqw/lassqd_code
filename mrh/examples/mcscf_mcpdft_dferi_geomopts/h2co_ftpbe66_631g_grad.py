...
from pyscf import scf, gto, lib, df
...
H  1.102430  0.000000 -0.920125'''
mol = gto.M (atom = h2co_casscf66_631g_xyz, basis = '6-31g', symmetry = False, verbose = lib.logger.INFO, output = 'h2co_ftpbe66_631g_grad.log')
mf_conv = scf.RHF (mol).run ()
mc_conv = mcpdft.CASSCF (mf_conv, 'ftPBE', 6, 6, grids_level=6)
...
try:
    de_num = np.load ('h2co_ftpbe66_631g_grad_num.npy')
    ...
    np.save ('h2co_ftpbe66_631g_grad_num.npy', np.stack ((de_conv_num, de_df_num), axis=0))
def my_call (env):
