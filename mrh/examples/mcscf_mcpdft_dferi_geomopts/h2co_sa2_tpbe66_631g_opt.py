...
H  1.102430  0.000000 -0.920125'''
mol = gto.M (atom = h2co_casscf66_631g_xyz, basis = '6-31g', symmetry = False, verbose = lib.logger.INFO, output = 'h2co_sa2_tpbe66_631g_opt.log')
...
mc_opt_df = mcpdft_grad.Gradients (mc_df).as_scanner (state = 0).optimizer ()
...
mc_opt_df = mcpdft_grad.Gradients (mc_df).as_scanner (state = 1).optimizer ()
mc_opt_df.callback = my_call
