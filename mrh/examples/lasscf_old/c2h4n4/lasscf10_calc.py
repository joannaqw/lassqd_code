...
from mrh.my_dmet.fragments import make_fragment_atom_list
from functools import reduce
import numpy as np
import re
import c2h4n4_struct

assert (len (sys.argv) > 1), "Usage: python lasscf10_calc.py dr_nn [options]"
# dr_nn is the shift in the N=N internuclear distance in Angstrom compared to the equilibrium geometry (1.249914)
# most important options:
#   load_scan_guess=dr_nn_guess
#       Load active orbitals and 1-RDM from a different point on the potential energy curve
#       dr_nn_guess is shift of N=N internuclear distance from another LASSCF(10,10) calculation,
#       which checkpoint file exists in the current directory.
#   load_CASSCF_guess
#       Load active orbitals from a comparable CASSCF calculation.
#       Requires file named "c2h4n4_cas10_ano.${dr_nn}.npy" in current directory.
...
print ("c2h4n4 at dr_nn = {}".format (dr_nn))
CASlist = None #np.empty (0, dtype=int)
ints = re.compile ("[0-9]+")
my_kwargs = {'calcname':           ('c2h4n4_lasscf10_dr' + ['{:02.0F}','{:03.0F}'][dr_nn < 0]).format (dr_nn*10),
             'doLASSCF':           True,
             'debug_energy':       False,
             'debug_reloc':        False,
...
for iargv in sys.argv[2:]:
    if iargv in my_kwargs and isinstance (my_kwargs[iargv], bool) and not my_kwargs[iargv]:
...
# --------------------------------------------------------------------------------------------------------------------
mol = c2h4n4_struct.structure (dr_nn, dr_nn, basis, symmetry=False)
mf = scf.RHF (mol)
mf.verbose = 4
mf.kernel ()
if not mf.converged:
...
C2H2 = make_fragment_atom_list (myInts, list (range(3,7)), 'CASSCF(2,2)', name='C2H2')
...
if load_casscf_guess:
    npyfile = 'c2h4n4_cas10_ano.{:.1f}.npy'.format (dr_nn)
    norbs_cmo = (mol.nelectron - 10) // 2
    norbs_amo = 10
    N2Ha.load_amo_guess_from_casscf_npy (npyfile, norbs_cmo, norbs_amo)
    C2H2.load_amo_guess_from_casscf_npy (npyfile, norbs_cmo, norbs_amo)
    N2Hb.load_amo_guess_from_casscf_npy (npyfile, norbs_cmo, norbs_amo)
elif dr_guess is not None:
    chkname = ('c2h4n4_lasscf10_dr' + ['{:02.0F}','{:03.0F}'][dr_guess < 0]).format (dr_guess*10)
    c2h4n4_dmet.load_checkpoint (chkname + '.chk.npy')
else:
    c2h4n4_dmet.generate_frag_cas_guess (mf.mo_coeff, CASlist)

# Calculation
# --------------------------------------------------------------------------------------------------------------------
energy_result = c2h4n4_dmet.doselfconsistent ()
c2h4n4_dmet.save_checkpoint (my_kwargs['calcname'] + '.chk.npy')
