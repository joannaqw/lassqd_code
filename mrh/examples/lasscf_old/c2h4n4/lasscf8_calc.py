...
assert (len (sys.argv) > 1), "Usage: python lasscf8_calc.py dr_nn [options]"
...
#       dr_nn_guess is shift of N=N internuclear distance from another LASSCF(8,8) calculation,
#       which checkpoint file exists in the current directory.
#   load_CASSCF_guess
#       Load active orbitals from a comparable CASSCF calculation.
#       Requires file named "c2h4n4_cas8_ano.${dr_nn}.npy" in current directory.
...
ints = re.compile ("[0-9]+")
my_kwargs = {'calcname':           ('c2h4n4_lasscf8_dr' + ['{:02.0F}','{:03.0F}'][dr_nn < 0]).format (dr_nn*10),
             'doLASSCF':           True,
             'debug_energy':       False,
             'debug_reloc':        False,
