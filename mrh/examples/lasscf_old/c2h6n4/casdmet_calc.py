...
assert (len (sys.argv) > 1), "Usage: python casdmet_calc.py dr_nn [options]"
...
#       dr_nn_guess is shift of N=N internuclear distance from another CAS-DMET calculation,
...
ints = re.compile ("[0-9]+")
my_kwargs = {'calcname':           ('c2h6n4_casdmet_dr' + ['{:02.0F}','{:03.0F}'][dr_nn < 0]).format (dr_nn*10),
             'do1SHOT':            True,
             'debug_energy':       False,
             'debug_reloc':        False,
             'nelec_int_thresh':   1e-3,
             'num_mf_stab_checks': 5}
