...
assert (len (sys.argv) > 1), "Usage: python lasscf_calc.py r_nn [options]"
# r_nn is N=N internuclear distance in Angstrom (equilibrium geometry = 1.24414799)
# most important options:
#   load_scan_guess=r_nn_guess
#       Load active orbitals, chemical potential, and correlation-potential matrix from a different point on the potential energy curve
#       r_nn_guess is N=N internuclear distance from another scCAS-DMET calculation,
...
ints = re.compile ("[0-9]+")
my_kwargs = {'calcname':           'me2n2_sccasdmet_r{:2.0f}'.format (r_nn*10),
             'debug_energy':       False,
             'debug_reloc':        False,
...
if load_casscf_guess:
    ...
    chkname = 'me2n2_sccasdmet_r{:2.0f}'.format (dr_guess*10)
