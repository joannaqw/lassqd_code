...
assert (len (sys.argv) > 1), "Usage: python casdmet1e_calc.py r_nn [options]"
# r_nn is N=N internuclear distance in Angstrom (equilibrium geometry = 1.24414799)
# most important options:
#   load_scan_guess=r_nn_guess
#       Load active orbitals from a different point on the potential energy curve
#       r_nn_guess is N=N internuclear distance from another single-embedding CAS-DMET calculation,
...
ints = re.compile ("[0-9]+")
my_kwargs = {'calcname':           'me2n2_1edmet_r{:2.0f}'.format (r_nn*10),
             'do1EMB':             True,
             'debug_energy':       False,
             'debug_reloc':        False,
