...
class ContractHamCI (stdm.LSTDM):
    ...
    def _init_vecs (self):
        hci_fr_pabq = []
        nfrags, _nroots, nbra = self.nfrags, self.nroots, self.nbra
    ...
    def _crunch_2c_(self, bra, ket, i, j, k, l, s2lt):
        ...
        (0, 2, 3)[s2lt] # aa, ba, bb -> when you populate the e1 <-> e2 permutation
        s11 = s2 // 2
        s12 = s2 % 2
        nelec_f_bra = self.nelec_rf[self.rootaddr[bra]]
        nelec_f_ket = self.nelec_rf[self.rootaddr[ket]]
...
def gen_contract_ham_ci_const (ifrag, nbra, las, h1, h2, ci, nelec_frs, soc=0, orbsym=None,
    ...
    lib.logger.new_logger (las, las.verbose)
