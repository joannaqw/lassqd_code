...
from mrh.my_pyscf.fci.spin_op import contract_sdown, mdown, mup
...
class SpinFlips (object):
    ...
    def __init__(self, ci, norb, nelec, spins, smults):
        ...
        for i in range (len (ci)):
            ci, _spin, smult = self.ci[i], self.spins[i], self.smults[i]
            if smult>1:
...
def charge_excitation_products (lsi, spaces, nroots_ref=0, space0=None):
    ...
    _i0, _j0 = i, j = nroots_ref, len (spaces)
...
class LASSIS (LASSI):
    ...
    def make_fbfdm1 (self, pfrag, si=None, state=0):
        from mrh.my_pyscf.lassi.sitools import decompose_sivec_by_rootspace, _trans_sdm1
