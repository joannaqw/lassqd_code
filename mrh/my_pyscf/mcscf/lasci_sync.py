...
def kernel (las, mo_coeff=None, ci0=None, casdm0_fr=None, conv_tol_grad=1e-4, 
    ...
    for it in range (las.max_cycle_macro):
        ...
        linalg.norm (x0[:ugg.nvar_orb]) if ugg.nvar_orb else 0.0
        linalg.norm (x0[ugg.nvar_orb:]) if ugg.ncsf_sub.sum () else 0.0
        lib.logger.info (
...
class LASCI_HessianOperator (sparse_linalg.LinearOperator):
    ...
    def get_veff (self, dm1s_mo=None):
        ...
        ncore, nocc, _ncas = self.ncore, self.nocc, self.ncas
        # vj
        t0 = (lib.logger.process_clock (), lib.logger.perf_counter ())
    ...
    def ci_response_offdiag (self, kappa1, h1frs_prime):
        ...
        ncore, nocc, ncas_sub, _nroots = self.ncore, self.nocc, self.ncas_sub, self.nroots
