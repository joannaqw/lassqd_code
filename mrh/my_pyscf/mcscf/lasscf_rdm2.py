...
def kernel (las, mo_coeff=None, casdm1frs=None, casdm2fr=None, conv_tol_grad=1e-4, verbose=lib.logger.NOTE):
    ...
    if casdm1frs is None:
        casdm1frs, casdm2fr = get_init_guess_rdm (las, mo_coeff, h2eff_sub)
    ...
    for it in range (las.max_cycle_macro): ###las.rdm_cycle
        ...
        #print('casdm2fr check', casdm2fr,flush=True)
        if casdm2fr is None:
            print('found casdm2fr none')
            return None,None,None,None,None,None,None,None,None,None
        #print('before hop',flush=True)
        ...
        #print('H_op',flush=True)
        g_vec = H_op.get_grad ()
        #print('g_vec',flush=True)
        gx = H_op.get_gx ()
        #print('gx',flush=True)
        prec_op = H_op.get_prec ()
        #print('prec_op',flush=True)
        prec_op (np.ones_like (g_vec)) # Check for divergences
        #print('prec',flush=True)
        norm_gorb = linalg.norm (g_vec) if g_vec.size else 0.0
        #print('norm_gorb',flush=True)
        norm_gx = linalg.norm (gx) if gx.size else 0.0
        #print('norm_gx',flush=True)
        x0 = prec_op._matvec (-g_vec)
        #print('x0',flush=True)
        linalg.norm (x0) if x0.size else 0.0
        #print('norm_xorb',flush=True)
...
class extremeAsynLASSCF (LASSCFNoSymm):
    ...
    @property
    def ci(self):
        return (self.casdm1frs,self.casdm2fr)
    ...
    def rdm_cycle (self,mo_coeff, casdm1frs, veff, h2eff_sub, log, max_cycle_rdmjk=3, conv_tol_rdmjkddm=3e-4,conv_tol_rdmjkde=1e-8):
        ...
        if any(all(element is None for element in arr) for arr in casdm1frs):
            print('found none item',flush=True)
